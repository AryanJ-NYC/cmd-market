...
export async function createPresignedUploadRequest({
  ...
  return { expiresAt, headers: { 'content-type': contentType }, method: 'PUT', url };
}

export function buildDraftAssetKey({ filename, listingId, uploadId }: BuildDraftAssetKeyInput) {
...
export async function assertUploadedAssetExists(assetKey: string) {
  try {
    await getSpacesClient().send(new HeadObjectCommand({ Bucket: SPACES_BUCKET, Key: assetKey }));
  } catch (error) {
